...
import uk.ac.gla.cvr.hoci.DateUtils;
...
public class CommandDocumentJsonUtils {
	...
	public static CommandDocument jsonObjectToCommandDocument(JsonObject jsonObject) {
		...
		if(jsonKeySet.size() != 1) {
			throw new RuntimeException("Root JSON object must have a single key");
		}
		...
		if(!(jsonValue instanceof JsonObject)) {
			throw new RuntimeException("Root JSON object must have an object as its single value");
		}
	...
	private static void populateChildCommandObjectFromJsonValue(CommandObject parentCommandObject, String key, JsonValue jsonValue) {
		if(jsonValue instanceof JsonObject) {
			...
			throw new RuntimeException("GLUE object may not contain JSON value "+jsonValue);
		}
	}
	...
	private static void addArrayItemFromJsonValue(CommandArray commandArray, JsonValue jsonValue) {
		if(jsonValue instanceof JsonObject) {
			...
			throw new RuntimeException("GLUE array may not contain JSON array as item");
		} else if(jsonValue.getValueType() == ValueType.NULL){
			...
			throw new RuntimeException("GLUE array may not contain JSON value "+jsonValue);
		}
	}
